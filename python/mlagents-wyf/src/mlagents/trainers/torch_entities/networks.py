...
from mlagents.trainers.torch_entities.decoders import ValueHeads, TaskSharedValueHeads, GraphLevelValueHeads
from mlagents.trainers.torch_entities.layers import LSTM, LinearEncoder, Swish
...
from mlagents.trainers.torch_entities.decoders import TaskSharedValueHeads, GraphLevelValueHeads
...
class NetworkBody(nn.Module):
    def __init__(
        ...
        forV: bool = False,  # 2025.10.09 wyf
    ):
        super().__init__()
        ...
        if (
            ...
            # layers = [nn.Linear(total_enc_size, self.h_size), nn.ReLU(inplace=True)]
            # for _ in range(network_settings.num_layers - 1):
            #     layers.append(nn.Linear(self.h_size, self.h_size))
            #     layers.append(nn.ReLU(inplace=True))

            # layers = [nn.Linear(total_enc_size, self.h_size), Swish()]
            # for _ in range(network_settings.num_layers - 1):
            #     layers.append(nn.Linear(self.h_size, self.h_size))
            #     layers.append(Swish())
...
class MultiAgentNetworkBody(torch.nn.Module):
    ...
    def __init__(
        ...
        self.linear_encoder = LinearEncoder(
            attention_embeding_size,      # input_size: int,
            network_settings.num_layers,  # num_layers: int,
            self.h_size,                  # hidden_size: int,
    ...
    def _get_masks_from_nans(self, obs_tensors: List[torch.Tensor]) -> torch.Tensor:
        ...
        # 这样得到的掩码中：1表示NaN（需要被掩码的位置），0表示有效数据
    ...
    def forward(
        ...
        encoded_entity = torch.cat(self_attn_inputs, dim=1)  # 15 x 7 x 512
        # print("encoded_entity.shape", encoded_entity.shape)
        # 注意力机制
        encoded_state = self.self_attn(encoded_entity, self_attn_masks)
        # print("encoded_state.shape", encoded_state.shape)

        flipped_masks = 1 - torch.cat(self_attn_masks, dim=1)  # 1表示有效，0表示无效
        num_agents = torch.sum(flipped_masks, dim=1, keepdim=True)  # 对每行求和，表示有效智能体个数，keepdim保持维度，便于后续广播
        ...
        # print("num_agents.shape:", num_agents.shape)
        ...
        if self.use_lstm:
            ...
            encoding, memories = self.lstm(encoding, memories)
            encoding = encoding.reshape([-1, self.m_size // 2])
        encoding = torch.cat([encoding, num_agents], dim=1)  # 将num_agents作为额外特征输入
        # print("encoding。shape:", encoding.shape)

        return encoding, memories

class MultiAgentNetworkBodyForMAPPO(torch.nn.Module):
    def __init__(
        self,
        observation_specs: List[ObservationSpec],
        network_settings: NetworkSettings,
        ...
        # VECTOR形式观测的observation_encoder没有神经网络，只做了归一化，这里添加一层原有线性层linear_encoder（输入维度是attention_embeding_size）之前的线性网络
        # 写一个linear_encoder_0将任意维度的输入转化成self.h_size
        self.linear_encoder_0 = LinearEncoder(
            # 871 * 7,                               # 初版pure-MAPPO时聚合7个智能体的信息（每个智能体观测维数871） 871*7
            # 1039 * 7,                                  # ANN-MAPPO baseline 2025.09.23
            ( 7 + 512 ) * 7,                        # RGCN-MAPPO 7来自智能体自身信息，512来自RGCN
            3,       # network_settings.num_layers,
            self.h_size,
            kernel_gain=(0.125 / self.h_size) ** 0.5,
            )
    ...
    def forward(
        ...
        if obs_only:
            ...
            for inputs in obs_only:
                # print(inputs.shape)
                encoded = self.observation_encoder(inputs)  # encoded.shape = 15 x 871  # 15会变，有时14，有时13，等等
                concat_encoded_obs.append(encoded)
            # g_inp = torch.stack(concat_encoded_obs, dim=1)  # g_inp.shape = 15 x 7 x 871
            g_inp = torch.cat(concat_encoded_obs, dim=1)  # g_inp.shape = 15 x ( 7 x 871 ) = 15 x 6097 for pure-MAPPO
            # print("g_inp.shape", g_inp.shape)             # g_inp.shape = 14 x ( 7 x (11+512) ) = 14 x 3661 for RGCN-MAPPO
        ...
        #
        # # num_agents will be -1 for a single agent and +1 when the current maximum is reached
        num_agents = num_agents * 2.0 / self._current_max_agents - 1  # 15 x 1

        # encoding = self.linear_encoder(encoded_state)
        encoding_0 = self.linear_encoder_0(g_inp)        #
        # print("encoding_0.shape", encoding_0.shape)
        encoding = self.linear_encoder(encoding_0)       #
        # print("encoding.shape", encoding.shape)
        # if self.use_lstm:
        #     # Resize to (batch, sequence length, encoding size)
        #     encoding = encoding.reshape([-1, sequence_length, self.h_size])
...
class ValueNetwork(nn.Module, Critic):
    def __init__(
        ...
        self.network_body = NetworkBody(
            observation_specs, network_settings, encoded_act_size=encoded_act_size, forV=True
        )
        if network_settings.memory is not None:
        ...
        # self.task_shared_value_heads = TaskSharedValueHeads(stream_names, encoding_size, 86)
        # self.graph_level_value_heads = GraphLevelValueHeads(stream_names,encoding_size)
    ...
    def forward(
        ...
        # output = self.task_shared_value_heads(encoding)  # 2025.09.27 wyf
        # output = self.graph_level_value_heads(encoding, actions)  # 2025.09.27 wyf
...
class SimpleActor(nn.Module, Actor):
    ...
    def __init__(
        ...
        self.network_body = NetworkBody(observation_specs, network_settings, forV = False)  # 2025.09.26 wyf 用于策略网络
        if network_settings.memory is not None:

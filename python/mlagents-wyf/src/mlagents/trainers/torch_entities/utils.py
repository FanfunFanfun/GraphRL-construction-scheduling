...
from mlagents.trainers.torch_entities.graph_encoder import GraphEncoder, GraphEncoder4V
...
class ModelUtils:
    ...
    # Graph observation extension. Author: wyf.
    VALID_GRAPH_PROP = frozenset(
        [(DimensionProperty.GRAPH, DimensionProperty.GRAPH)]  # 与C#端通过数字5互认
    )
    ...
    def get_encoder_for_obs(
        ...
        # # GRAPH 0225.09.21 wyf
        if dim_prop in ModelUtils.VALID_GRAPH_PROP:
            if forV:
                return (
                    GraphEncoder4V(shape[0], shape[1], h_size, h_size),
                    h_size,
                )
            return (GraphEncoder(shape[0], shape[1], h_size, h_size), h_size)
        # OTHER
    ...
    def create_input_processors(
        ...
        for obs_spec in observation_specs:
            encoder, embedding_size = ModelUtils.get_encoder_for_obs(
                obs_spec, normalize, h_size, attention_embedding_size, vis_encode_type, forV = forV,
            )

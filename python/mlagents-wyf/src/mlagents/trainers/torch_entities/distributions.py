...
class MultiCategoricalDistribution(nn.Module):
    def __init__(self, hidden_size: int, act_sizes: List[int]):
        ...
        # Scheduling graph extension. Author: wyf.
        self.task_shared_branches = self._create_task_shared_policy_branches(
            hidden_size
        )
    ...
    def _create_task_shared_policy_branches(self, hidden_size: int) -> nn.ModuleList:
        return nn.ModuleList(nn.Linear(hidden_size, 1) for _ in self.act_sizes)
    ...
    def forward(self, inputs: torch.Tensor, masks: torch.Tensor) -> List[DistInstance]:
        ...
        graph_input = inputs.ndim == 3
        branches = self.task_shared_branches if graph_input else self.branches
        for idx, branch in enumerate(branches):
            if graph_input:
                if inputs.shape[1] != self.act_sizes[idx]:
                    raise ValueError(
                        f"Graph action branch {idx} has {self.act_sizes[idx]} actions, "
                        f"but the encoder produced {inputs.shape[1]} task embeddings."
                    )
                logits = branch(inputs).squeeze(-1)
            else:
